...
contract Dice {
    ...
    function add(
        ...
        require(msg.value > .01 ether, "at least 0.01 ETH is needed to spawn a new dice");
    ...
    // get number of luckytimes
    function luckytimesEvent(uint256 diceId) public view validDiceId(diceId) returns (uint256) {
        return dices[diceId].luckyTimes;
    }

    function getLuckytimesValue(uint256 diceId) public view validDiceId(diceId) returns (uint256) {
        return dices[diceId].numberOfSides;
    }

    function destroyDice(uint256 diceId) payable public validDiceId(diceId) ownerOnly(diceId){
